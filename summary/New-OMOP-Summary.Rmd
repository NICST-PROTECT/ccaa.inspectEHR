...
raw_sql <- read_file("summary//measurement_observation_summary.sql") %>% 
    str_replace_all(":OMOP_SCHEMA", "cca_omop")
measurement_observation_summary <- dbGetQuery(omop_conn, raw_sql) %>%
    as_tibble()

raw_sql <- read_file("summary//condition_procedure_summary.sql") %>% 
    str_replace_all(":OMOP_SCHEMA", "cca_omop")
condition_procedure_summary <- dbGetQuery(omop_conn, raw_sql) %>%
    as_tibble()

raw_sql <- read_file("summary//drug_summary.sql") %>% 
    str_replace_all(":OMOP_SCHEMA", "cca_omop")
