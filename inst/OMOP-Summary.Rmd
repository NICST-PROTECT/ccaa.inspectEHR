...
raw_sql <- read_file(system.file("measurement_observation_summary.sql", package = "ccaa.inspectEHR")) %>% 
    str_replace_all(":OMOP_SCHEMA", params$schema)
measurement_observation_summary <- dbGetQuery(ctn, raw_sql) %>%
    as_tibble()

raw_sql <- read_file(system.file("condition_procedure_summary.sql", package = "ccaa.inspectEHR")) %>% 
    str_replace_all(":OMOP_SCHEMA", params$schema)
condition_procedure_summary <- dbGetQuery(ctn, raw_sql) %>%
    as_tibble()

raw_sql <- read_file(system.file("drug_summary.sql", package = "ccaa.inspectEHR")) %>% 
    str_replace_all(":OMOP_SCHEMA", params$schema)
...
summary <- bind_rows(summary, c(c(variable = "Number of males"), value = nrow(st[["person"]] %>% filter(gender_concept_id == "MALE"))))
summary <- bind_rows(summary, c(c(variable = "Number of females"), value = nrow(st[["person"]] %>% filter(gender_concept_id == "FEMALE"))))
...
```{r admission-from2}
st[["visit_occurrence"]] %>%
  # mutate(admitted_from_concept_id = factor(
  #   x = admitted_from_concept_id, levels = asci$concept_name, labels = asci$concept_name
  # )) %>%
...
```{r discharge-to2}
st[["visit_occurrence"]] %>%
  #   mutate(discharged_to_concept_id = factor(
  #   x = discharged_to_concept_id, levels = dtci$concept_name, labels = dtci$concept_name
  # )) %>%
