...
observation_period <- observation_period %>% filter(person_id %in% get_period_patients(ctn, schema, params$start_date, params$end_date))

visit_detail <- tbl(ctn, in_schema(schema, "visit_detail")) %>%
  as_tibble() %>%
  filter(person_id %in% get_period_patients(ctn, schema, params$start_date, params$end_date)) %>%
  get_concept_names(ctn, schema)

st <- prepare_tables(ctn, schema) %>%
  map(~
    filter(., person_id %in% get_period_patients(ctn, schema, params$start_date, params$end_date)))

care_site <- tbl(ctn, in_schema(schema, "care_site")) %>% as_tibble()
...
```{r visit-profile}
plot_visit_profile(st[["visit_occurrence"]], params$start_date, params$end_date, params$custom_colors) %>% custom_theme(params$custom_colors)
```
