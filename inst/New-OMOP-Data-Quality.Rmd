...
measure_bounds = readr::read_csv(file ='./../data/measure_bounds.csv')

question_mapping = readr::read_csv(file ='./../data/question_mappings.csv')

```
<br>
<font size="4"> Reporting Period : `r {params$start_date}` - `r {params$end_date}` </font> 
...
if (nrow(duplicates)>0) {
    ...
    fail_percentage = round(fail_count/nrow(st[["person"]])*100,2)
    data_qual_df = add_new_check(data_qual_df,check,'Fail',fail_count,fail_percentage)
  } else {
    data_qual_df = add_new_check(data_qual_df,check,'Pass','0','0')
  }
...
if (nrow(death_today)>0) {
    ...
    fail_percentage = round(fail_count/nrow(st[["death"]])*100,2)
    data_qual_death = add_new_check(data_qual_death,check,'Fail',fail_count,fail_percentage)
  } else {
    data_qual_death = add_new_check(data_qual_death,check,'Pass','0','0')
  }
...
```{r}
data_qual_measure = data.frame(matrix(ncol = 3, nrow = 0))
colnames(data_qual_measure) = c('Check','Status','If check fails : n(%)')
```
...
check = 'Heart rate is within 0-300 level'
if (nrow(abnormal_heart)>0) {
    fail_count = nrow(abnormal_heart)
    fail_percentage = round(fail_count/nrow(heart_rate)*100,2)
    data_qual_measure = add_new_check(data_qual_measure,check,'Fail',fail_count,fail_percentage)
  } else {
    data_qual_measure = add_new_check(data_qual_measure,check,'Pass','0','0')
  }
