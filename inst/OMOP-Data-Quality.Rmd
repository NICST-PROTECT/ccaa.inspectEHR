---
title: "CCAA Repository Data Quality Report"
...
tolerance <- params$measurement_tolerance
...
measurements = tbl(ctn, in_schema(schema, "measurement")) %>% as_tibble() %>% filter(person_id %in% get_period_patients(observation_period, params$start_date, params$end_date))
...
question_mapping = read_csv(file =system.file("data", "question_mappings.csv", package = 'ccaa.inspectEHR'))

concepts = tbl(ctn, in_schema(schema, "concept")) %>% as_tibble()
...
## Measurements
The status of the pass/failure of the measurement checks undergo with `r {tolerance}`% tolerance. 
```{r}
...
heart_rate = measurements%>% filter(measurement_concept_id== get_source_concept_id('heart_rate',question_mapping))

abnormal_heart = check_measure_bounds('heart_rate',measure_bounds,heart_rate)

check = glue('Heart rate is within 0-450 bpm level')

if (nrow(abnormal_heart)>0) {
  fail_count = nrow(abnormal_heart)
  fail_percentage = format(round(fail_count/nrow(heart_rate)*100,4), scientific = FALSE)
  if (as.numeric(fail_percentage) > as.numeric(tolerance)) {
    data_qual_measure = add_new_check(data_qual_measure,nrow(heart_rate),check,'Fail',fail_count,fail_percentage)
  }else{
    data_qual_measure = add_new_check(data_qual_measure,nrow(heart_rate),check,'Pass',fail_count,fail_percentage)
  }
  } else {
    data_qual_measure = add_new_check(data_qual_measure,nrow(heart_rate),check,'Pass','0','0')
  }
...
temperature = measurements%>% filter(measurement_concept_id== get_source_concept_id('temperature',question_mapping))

abnormal_temp = check_measure_bounds('temperature',measure_bounds,temperature)

check = glue('Temperature is within 32-115 °F level')

if (nrow(abnormal_temp)>0) {
  fail_count = nrow(abnormal_temp)
  fail_percentage = format(round(fail_count/nrow(temperature)*100,4), scientific = FALSE)
  if (as.numeric(fail_percentage) > as.numeric(tolerance)) {
    data_qual_measure = add_new_check(data_qual_measure,nrow(temperature),check,'Fail',fail_count,fail_percentage)
  }else{
    data_qual_measure = add_new_check(data_qual_measure,nrow(temperature),check,'Pass',fail_count,fail_percentage)
  }
  } else {
    data_qual_measure = add_new_check(data_qual_measure,nrow(temperature),check,'Pass','0','0')
  }
...
respiratory_rate = measurements%>% filter(measurement_concept_id== get_source_concept_id('respiratory_rate',question_mapping))

abnormal_rr = check_measure_bounds('respiratory_rate',measure_bounds,respiratory_rate)

check = glue('Repository rate is within 0-100 bpm level')

if (nrow(abnormal_rr)>0) {
  fail_count = nrow(abnormal_rr)
  fail_percentage = format(round(fail_count/nrow(respiratory_rate)*100,4), scientific = FALSE)
  if (as.numeric(fail_percentage) > as.numeric(tolerance)) {
    data_qual_measure = add_new_check(data_qual_measure,nrow(respiratory_rate),check,'Fail',fail_count,fail_percentage)
  }else{
    data_qual_measure = add_new_check(data_qual_measure,nrow(respiratory_rate),check,'Pass',fail_count,fail_percentage)
  }
  } else {
    data_qual_measure = add_new_check(data_qual_measure,nrow(respiratory_rate),check,'Pass','0','0')
  }
...
diastolic_blood_pressure = measurements %>%filter(measurement_concept_id== get_source_concept_id('diastolic_blood_pressure',question_mapping))

abnormal_dbp = check_measure_bounds('diastolic_blood_pressure',measure_bounds,diastolic_blood_pressure)

check = glue('Diastolic blood pressure rate is within 0-200 mmHg level')

if (nrow(abnormal_dbp)>0) {
  fail_count = nrow(abnormal_dbp)
  fail_percentage = format(round(fail_count/nrow(diastolic_blood_pressure)*100,4), scientific = FALSE)
  if (as.numeric(fail_percentage) > as.numeric(tolerance)) {
    data_qual_measure = add_new_check(data_qual_measure,nrow(diastolic_blood_pressure),check,'Fail',fail_count,fail_percentage)
  }else{
    data_qual_measure = add_new_check(data_qual_measure,nrow(diastolic_blood_pressure),check,'Pass',fail_count,fail_percentage)
  }
  } else {
    data_qual_measure = add_new_check(data_qual_measure,nrow(diastolic_blood_pressure),check,'Pass','0','0')
  }
...
systolic_blood_pressure = measurements%>% filter(measurement_concept_id== get_source_concept_id('systolic_blood_pressure',question_mapping))

abnormal_sbp = check_measure_bounds('systolic_blood_pressure',measure_bounds,systolic_blood_pressure)

check = glue('Systolic blood pressure is within 0-400 mmHg level')

if (nrow(abnormal_sbp)>0) {
  fail_count = nrow(abnormal_sbp)
  fail_percentage = format(round(fail_count/nrow(systolic_blood_pressure)*100,4), scientific = FALSE)
  if (as.numeric(fail_percentage) > as.numeric(tolerance)) {
    data_qual_measure = add_new_check(data_qual_measure,nrow(systolic_blood_pressure),check,'Fail',fail_count,fail_percentage)
  }else{
    data_qual_measure = add_new_check(data_qual_measure,nrow(systolic_blood_pressure),check,'Pass',fail_count,fail_percentage)
  }
  } else {
    data_qual_measure = add_new_check(data_qual_measure,nrow(systolic_blood_pressure),check,'Pass','0','0')
  }

```

```{r print measurement checks}
data_qual_measure %>% print_large_kable()%>%
  column_spec(1,width='55%') %>%
  column_spec(2:ncol(glossary),width='15%')
```
